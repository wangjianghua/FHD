...
{
  vuint32_t otg_ctl;      /* USB_OTG Control and Status Register    000h*/
  vuint32_t otg_int;      /* USB_OTG Interrupt Register             004h*/
  vuint32_t ahb_cfg;      /* Core AHB Configuration Register    008h*/
  vuint32_t usb_cfg;      /* Core USB Configuration Register    00Ch*/
  vuint32_t rst_ctl;      /* Core Reset Register                010h*/
  vuint32_t int_sts;      /* Core Interrupt Register            014h*/
  vuint32_t int_msk;      /* Core Interrupt Mask Register       018h*/
  vuint32_t rx_stsr;      /* Receive Sts Q Read Register        01Ch*/
  vuint32_t rx_stsp;      /* Receive Sts Q Read & POP Register  020h*/
  vuint32_t rx_fifo_siz;  /* Receive FIFO Size Register         024h*/
  vuint32_t np_tx_fifo_siz;   /* Non Periodic Tx FIFO Size Register 028h*/
  vuint32_t np_tx_sts;    /* Non Periodic Tx FIFO/Queue Sts reg 02Ch*/
  vuint32_t i2c_ctl;      /* I2C Access Register                030h*/
  vuint32_t phy_vnd_ctl;  /* PHY Vendor Control Register        034h*/
  vuint32_t gpio;         /* General Purpose IO Register        038h*/
  vuint32_t usr_id;       /* User ID Register                   03Ch*/
  vuint32_t snps_id;      /* Synopsys ID Register               040h*/
  vuint32_t hw_cfg1;      /* User HW Config1 Register (RO)      044h*/
  vuint32_t hw_cfg2;      /* User HW Config2 Register (RO)      048h*/
  vuint32_t hw_cfg3;      /* User HW Config3 Register (RO)      04Ch*/
  vuint32_t hw_cfg4;      /* User HW Config4 Register (RO)      050h*/
  uint32_t  reserved[43];          /* Reserved                      054h-0FFh*/
  vuint32_t host_p_tx_fifo_siz; /* Host Periodic Tx FIFO Size Reg     100h*/
  vuint32_t dev_p_tx_fsiz_dieptxf[15];/* dev Periodic Transmit FIFO */
}
USB_OTG_common_regs;
...
{
  vuint32_t dev_cfg;         /* dev Configuration Register   800h*/
  vuint32_t dev_ctl;         /* dev Control Register         804h*/
  vuint32_t dev_sts;         /* dev Status Register (RO)     808h*/
  uint32_t reserved3;         /* Reserved                     80Ch*/
  vuint32_t dev_in_ep_msk;   /* dev IN Endpoint Mask         810h*/
  vuint32_t dev_out_ep_msk;  /* dev OUT Endpoint Mask        814h*/
  vuint32_t dev_all_int;     /* dev All Endpoints Itr Reg    818h*/
  vuint32_t dev_all_int_msk; /* dev All Endpoints Itr Mask   81Ch*/
  uint32_t  Reserved8;       /* Reserved                     820h*/
  vuint32_t Reserved9;       /* Reserved                     824h*/
  vuint32_t dev_vbus_dis;    /* dev VBUS discharge Register  828h*/
  vuint32_t dev_vbus_pulse;  /* dev VBUS Pulse Register      82Ch*/
  vuint32_t dev_thr_ctl;     /* dev thr                      830h*/
  vuint32_t dev_fifo_empty_msk; /* dev empty msk             834h*/
} USB_OTG_dev_regs;
/**
  * @}
  */ 
...
{
  vuint32_t dev_in_ep_ctl; /* dev IN Endpoint Control Reg 900h + (ep_num * 20h) + 00h*/
  uint32_t reserved04;             /* Reserved                       900h + (ep_num * 20h) + 04h*/
  vuint32_t dev_in_ep_int; /* dev IN Endpoint Itr Reg     900h + (ep_num * 20h) + 08h*/
  uint32_t reserved0C;             /* Reserved                       900h + (ep_num * 20h) + 0Ch*/
  vuint32_t dev_in_ep_txfer_siz; /* IN Endpoint Txfer Size   900h + (ep_num * 20h) + 10h*/
  vuint32_t dev_in_ep_dma; /* IN Endpoint DMA Address Reg    900h + (ep_num * 20h) + 14h*/
  vuint32_t dev_tx_fifo_sts;/*IN Endpoint Tx FIFO Status Reg 900h + (ep_num * 20h) + 18h*/
...
{
  vuint32_t dev_out_ep_ctl;       /* dev OUT Endpoint Control Reg  B00h + (ep_num * 20h) + 00h*/
  vuint32_t dev_out_ep_frm_nbr;   /* dev OUT Endpoint Frame number B00h + (ep_num * 20h) + 04h*/
  vuint32_t dev_out_ep_int;              /* dev OUT Endpoint Itr Reg      B00h + (ep_num * 20h) + 08h*/
  uint32_t reserved0C;                    /* Reserved                         B00h + (ep_num * 20h) + 0Ch*/
  vuint32_t dev_out_ep_txfer_siz; /* dev OUT Endpoint Txfer Size   B00h + (ep_num * 20h) + 10h*/
  vuint32_t dev_out_ep_dma;              /* dev OUT Endpoint DMA Address  B00h + (ep_num * 20h) + 14h*/
...
{
  vuint32_t host_cfg;             /* Host Configuration Register    400h*/
  vuint32_t host_frm_intrvl;      /* Host Frame Interval Register   404h*/
  vuint32_t host_frm_nbr;         /* Host Frame Nbr/Frame Remaining 408h*/
  uint32_t reserved40C;                   /* Reserved                       40Ch*/
  vuint32_t host_p_tx_sts;   /* Host Periodic Tx FIFO/ Queue Status 410h*/
  vuint32_t host_all_int;   /* Host All Channels Interrupt Register 414h*/
  vuint32_t host_all_int_msk;   /* Host All Channels Interrupt Mask 418h*/
}
USB_OTG_host_regs;
/**
  * @}
...
typedef struct _USB_OTG_hc_regs
{
  ...
  vuint32_t hc_split;
  vuint32_t hc_int;
  vuint32_t hc_int_msk;
  vuint32_t hc_txfer_siz;
  vuint32_t hc_dma;
  uint32_t reserved[2];
} USB_OTG_hc_regs;
/**
...
typedef struct USB_OTG_core_regs //000h
{
  ...
  vuint32_t                        *hprt0;
  vuint32_t                        *data_fifo[MAX_EPS_CHANNELS];
  vuint32_t                        *pcgcctl;
}
